...
VAULT_TOKEN=$(vault login -method=userpass username=$VAULT_USER | grep -o 'hvs\.[^[:space:]]*')
...
if [ -n "$VAULT_SSH_KEY" ]; then
  ...
  vault write -field=signed_key ssh-client-signer/sign/ssh_admin public_key=@${VAULT_SSH_KEY}.pub valid_principals=$VAULT_USER > $VAULT_SIGNED_KEY
  if [ $? -ne 0 ]; then
